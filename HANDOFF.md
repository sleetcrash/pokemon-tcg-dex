...
- Size pickers (`.bsize`, binder and pocket) are `position:sticky` at the header height plus 4px, z-index 30 (under the header's 40), so the picker stays reachable while a long pocket scrolls; the button covers whatever tile corner scrolls under it. Verified: picker top holds at header bottom after scrolling, the button is the element under its own centre, and the popover opens while pinned.

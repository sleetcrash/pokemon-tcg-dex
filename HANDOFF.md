...
Game Boy Pokedex scheme in BOTH modes: light (paper ground, black ink, perforated header divider) and dark (same palette inverted via html[data-theme] token overrides), toggled by the crescent button, persisted in prefs.theme. Compact content-width header. Filter row is filters only (Gen/Region/Forms/Status); Forms = checkboxes ordered Base, Base Variations (renamed from Other forms), Regionals, Megas, Dynamax, Gigantamax, Tera, DEFAULT = Base only; entry-mode and placement radios live under the gear with Manage lists. Binder sprite slots: name/number header, art box, tags beneath, non-interactive 26px Pokeball status in the middle (matches list slot size), card-search icon lower-left. Final binder page pads with blank slots; size picker = hover grid behind a four-square icon. Pocket view = third view (card-shaped icon), identical tiles to binder in one vertical scroll, columns 2-6 via one-row picker, touch pinch, or ctrl+wheel trackpad pinch. Tera matching expanded (subtype + set/name alt-art expansion; Tera Charizard = 4 printings). Long-press = Pending on every input; right-click is a reserved no-op.
